...
std::vector<double> findZerosPoly4(std::complex<double> a,
...
rtrace::Collision Torus::intersect(const rtrace::Ray &ray) const {
    ...
    std::vector<double> tSolutions =
        findZerosPoly4(J * J, 2. * J * K, 2. * J * L + K * K - G, 2. * K * L - H, L * L - I);

    if(!tSolutions.size()) {
        return rtrace::Collision();
    }

    std::remove_if(tSolutions.begin(), tSolutions.end(),
                   [](const double &solution) { return solution < rtrace::EPSILON; });
