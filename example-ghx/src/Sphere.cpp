...
Sphere::Sphere(rtrace::Vector3 center, double radius, rtrace::Material material)
...
rtrace::Collision Sphere::intersect(const rtrace::Ray &ray) const {
    ...
    const double c =
        rtrace::length(ray.origin - center) * rtrace::length(ray.origin - center) - radius * radius;
