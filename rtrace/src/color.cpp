...
Color Color::operator*(const double &alpha) const {
    return Color(alpha * this->r, alpha * this->g, alpha * this->b, this->w);
}

Color Color::operator+(const Color &rhs) const {
    return Color((this->w * this->r + rhs.w * rhs.r) / 255,
                 (this->w * this->g + rhs.w * rhs.g) / 255,
                 (this->w * this->b + rhs.w * rhs.b) / 255, std::max(this->w, rhs.w));
}
